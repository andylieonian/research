            ...
            c.events = function() {
                ...
                c.addClass(d.body, "laydate_body"),
                    ...
                    c.on(h.oclear, "click",
                        function() {
                            c.elem[h.elemv] = "";
                            if(c.options.ctrl) {c.options.ctrl.$setViewValue("");}
                            if(c.options.jpath) {parent.formulaEngine.apply(c.options.jpath, "");viewEngine.tipsForVerify(document.body);}
                            c.close();
                        }),
                    ...
                    c.on(h.otoday, "click",
                        function() {
                            ...
                            if(c.options.ctrl) {c.options.ctrl.$setViewValue(laydate.now(0, c.options.format))}
                            if(c.options.jpath) {parent.formulaEngine.apply(c.options.jpath, "");viewEngine.tipsForVerify(document.body);}
                            c.close();
                        }),
                    h.ok = b("#laydate_ok"),
